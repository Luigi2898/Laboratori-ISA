                    ...
                    for i = 1:Ninputs_extended
                        if (tmp_col.val(i) == 1 && tmp_col.state(i) == "unused")
                            ...
                            if (sum(triplet_items) == 3)
                                ...
                                fprintf(fID,'\tFA%d_L%d : FA port map (%s,%s,%s,%s,%s);\n',FA_count,l,tmp_col.name(triplet_idx(1)),tmp_col.name(triplet_idx(2)),tmp_col.name(triplet_idx(2)),sum_name,carry_name)
...
fprintf(fID,'begin\n');
fprintf(fID,'\tSUM <= std_logic_vector(signed(FINAL_SUM(0)(65 downto 1)) + signed(FINAL_SUM(1)(65 downto 1)));\n');
